...
void handleClick(int x, int y) {
    if (menu_active) {
        ...
        if ((y > menu_height - 4 * button_size_y)
            ...
            menu_active = false;
            flurry_end_timed_event("Show Menu", NULL, NULL, 0);
            flurry_log_event("Hide Menu", 1);
        }
        ...
        if(colorViewed != NULL)
        {
        	// Log non-timed event with parameters
        	const char *aParamKeys= {"color"};
        	const char *aParamValues= {colorViewed};

        	flurry_log_event_with_params("Color Viewed", 0, &aParamKeys, &aParamValues , 1);
        }
    }
}
...
static void handleNavigatorEvent(bps_event_t *event) {
    ...
    case NAVIGATOR_SWIPE_DOWN:
        ...
        flurry_end_timed_event("Hide Menu", NULL, NULL, 0);
        flurry_log_event("Show Menu", 1);
        break;
    case NAVIGATOR_EXIT:
        shutdown = true;
        break;
...
int resize(bps_event_t *event) {
    ...
    if (event) {
        ...
        const char *aParamKeys= {"orientation"};
        const char *aParamValues= {orientation};

        flurry_log_event_with_params("Orientation Update", 0, &aParamKeys, &aParamValues , 1);
    }
...
int main(int argc, char *argv[]) {
    ...
    flurry_set_debug_log_enabled(1);
    flurry_start_session("YOUR_API_KEY", NULL, NULL, 0);
